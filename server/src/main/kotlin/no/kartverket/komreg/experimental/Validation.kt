...
sealed interface Validation<out A> : no.kartverket.komreg.experimental.LoggingContext<Validation<A>> {
    ...
    fun <B> fold(
        fe: (kotlinx.collections.immutable.PersistentList<no.kartverket.komreg.experimental.LoggingContext.LogRecord>, Nel<no.kartverket.komreg.experimental.LoggingContext.LogRecord>) -> B,
        fa: (kotlinx.collections.immutable.PersistentList<no.kartverket.komreg.experimental.LoggingContext.LogRecord>, A) -> B,
    ): B
    ...
    companion object {
        ...
        inline fun <A, B, C, R> productMap(
            fa: Validation<A>,
            fb: Validation<B>,
            fc: Validation<C>,
            crossinline f: (A, B, C) -> R,
        ): Validation<R> =
            productMap(productMap(fa, fb) { a, b -> { c: C -> f(a, b, c) } }, fc) { ff, c -> ff(c) }

        inline fun <A, B, C, D, R> productMap(
            fa: Validation<A>,
            fb: Validation<B>,
            fc: Validation<C>,
            fd: Validation<D>,
            crossinline f: (A, B, C, D) -> R,
        ): Validation<R> =
            productMap(productMap(fa, fb, fc) { a, b, c -> { d: D -> f(a, b, c, d) } }, fd) { ff, d -> ff(d) }

        inline fun <A, B, C, D, E_, R> productMap(
            fa: Validation<A>,
            fb: Validation<B>,
            fc: Validation<C>,
            fd: Validation<D>,
            fe: Validation<E_>,
            crossinline f: (A, B, C, D, E_) -> R,
        ): Validation<R> =
            productMap(productMap(fa, fb, fc, fd) { a, b, c, d -> { e: E_ -> f(a, b, c, d, e) } }, fe) { ff, e -> ff(e) }

        inline fun <A, B, C, D, E, F, R> productMap(
            fa: Validation<A>,
            fb: Validation<B>,
            fc: Validation<C>,
            fd: Validation<D>,
            fe: Validation<E>,
            ff: Validation<F>,
            crossinline f: (A, B, C, D, E, F) -> R,
        ): Validation<R> =
            productMap(productMap(fa, fb, fc, fd, fe) { a, b, c, d, e -> { f: F -> f(a, b, c, d, e, f) } }, ff) { fff, f1 ->
                fff(
                    f1,
                )
            }
