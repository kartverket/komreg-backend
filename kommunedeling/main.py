...
def createGrunndataTilFordeling(
    envUrl: str, kommunenr: str, vegadresser: List[str] = None
):
        ...
        if response.status_code == 200:
            ...
            return next(
                (
                    item
                    for item in res
                    if item["fylkesnummer"] == fylkesnummer
                    and item["kommunenummer"] == lopenummer
                ),
                None,
            )
        else:
    ...
    for i in range(1, len(grunndata["gårdsnumre"])):
        ...
        if gardsnummer - int(grunndata["gårdsnumre"][i - 1]) != 1:
            godkjenteGardsnummere.append(
                {"fra": gardsnummer_start, "til": int(grunndata["gårdsnumre"][i - 1])}
            )
            gardsnummer_start = gardsnummer

    # Capture the last serie after the loop completes
    godkjenteGardsnummere.append({"fra": gardsnummer_start, "til": gardsnummer})
...
# Paths for fordelingsparametre, regulering og kommunedata
...
kommunenr = (
    fordeling_input["fylkesnummer"] + fordeling_input["eksisterende_kommuneløpenummer"]
)
...
transformasjoner = createRegulering(
    fordeling_input=fordeling_input,
    grunndata=createGrunndataTilFordeling(valgtEnv["url"], kommunenr),
)
