...
sealed class Product<out A : Product<A>> {
    ...
    class Mirror<@Suppress("unused") out A : Product<*>> private constructor(
        ...
        companion object {
            ...
            inline fun <reified A, reified B, reified C> length3() : Mirror<And<And<A, B>, C>> = Unsafe.mirrorOf(typeOf<A>(), typeOf<B>(), typeOf<C>())
            inline fun <reified A, reified B, reified C, reified D> length4() : Mirror<And<And<And<A, B>, C>, D>> = Unsafe.mirrorOf(typeOf<A>(), typeOf<B>(), typeOf<C>(), typeOf<D>())
            inline fun <reified A, reified B, reified C, reified D, reified E> length5() : Mirror<And<And<And<And<A, B>, C>, D>, E>> = Unsafe.mirrorOf(typeOf<A>(), typeOf<B>(), typeOf<C>(), typeOf<D>(), typeOf<E>())
...
inline infix fun <reified A, reified B> A.productAnd(b: B): And<A, B> = Product.And(nonEmptyListOf(typeOf<A>() to this, typeOf<B>() to b))
