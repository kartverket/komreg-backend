...
class AdressenummerbokstavTest : AnnotationSpec() {
    ...
    fun `skal serialisere til 0 for adressenummerbokstav naar denne er NONE`() {
        val ident: AdressenummerbokstavIdent =
            AdressenummerbokstavIdent(Adressenummerbokstav(Adressenummerbokstav.NONE))

        assertEquals(Adressenummerbokstav(Adressenummerbokstav.NONE), ident.getOrNull<Adressenummerbokstav>())

        val json = json.encodeToString(Ident.serializer(), ident)

        assertEquals(
            """{"type":["no.kartverket.komreg.core.domain.Adressenummerbokstav"],"values":[{"type":"no.kartverket.komreg.core.domain.Adressenummerbokstav","value":"0"}]}""",
            json
        )
